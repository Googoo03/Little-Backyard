...
namespace Simplex
{
    ...
    public class Noise
    {
        ...
        //Create a 3D texture with custom specifications
        public void CreateTexture3D(int resolution, int octaves, float frequency, float lacunarity, float persistence, float amplitude, string filename = "Example3DPerlinTexture")
        {
            ...
            for (int z = 0; z < resolution; z++)
            {
                ...
                for (int y = 0; y < resolution; y++)
                {
                    ...
                    {
                        float localFrequency = 1f;
                        float localAmplitude = amplitude;
                        float totalValue = 0;
                        float value = 0;
                        for (int i = 0; i < octaves; ++i)
                        {
                            totalValue += localAmplitude;
                            // wrap normalized coordinate after scaling
                            float sx = Mathf.Repeat(x * localFrequency, resolution);
                            float sy = Mathf.Repeat(y * localFrequency, resolution);
                            float sz = Mathf.Repeat(z * localFrequency, resolution);
                            value += CalcPixel3D(sx, sy, sz) * localAmplitude;
                            localFrequency *= lacunarity;
                            localAmplitude *= persistence;
                        }
                        value /= totalValue > 0 ? totalValue : 1;

                        colors[x + yOffset + zOffset] = new Color(value, value, value, value);
                    }
                }
            }
            ...
            UnityEditor.AssetDatabase.CreateAsset(texture, "Assets/" + filename + ".asset");
        }
        ...
        public Vector3 Compute3DGradient(float x, float y, float z)
        {
            ...
            var grad0 = new Vector2(0, 0);
            var grad1 = new Vector2(0, 0);
            var grad2 = new Vector2(0, 0);
            var grad3 = new Vector2(0, 0);


            if (t0 >= 0.0f)
            {
                t0 *= t0;
                grad0 = Gradient3D(_perm[ii + _perm[jj + _perm[kk]]], x0, y0, z0);
            }
            if (t1 >= 0.0f)
            {
                t1 *= t1;
                grad1 = Gradient3D(_perm[ii + i1 + _perm[jj + j1 + _perm[kk + k1]]], x1, y1, z1);
            }
            if (t2 >= 0.0f)
            {
                t2 *= t2;
                grad2 = Gradient3D(_perm[ii + i2 + _perm[jj + j2 + _perm[kk + k2]]], x2, y2, z2);
            }
            if (t3 >= 0.0f)
            {
                t3 *= t3;
                grad3 = Gradient3D(_perm[ii + 1 + _perm[jj + 1 + _perm[kk + 1]]], x3, y3, z3);
            }



            return grad0 * t0 + grad1 * t1 + grad2 * t2 + grad3 * t3;
        ...
        private Vector2 Gradient3D(int hash, float x, float y, float z)
        {
